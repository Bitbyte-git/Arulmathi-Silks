...
const colorMap = {
  'Olive Green': '#647447', 'Antique Gold': '#b9842e', Maroon: '#681d24', 'Bottle Green': '#154a33', 'Temple Green': '#1f5a3d', Emerald: '#0f6a46',
  'Rose Pink': '#d87591', Coral: '#d75b48', Ivory: '#f1eadc', 'Champagne Gold': '#d7b46a',
  'Peacock Blue': '#0f6070', 'Teal Blue': '#0c6b76', 'Midnight Navy': '#111c3a',
  'Mint Gold': '#abc493', Sage: '#9aaa87',
  ...
  'Ruby Red': '#9e1f1d', 'Brick Red': '#8e3226', Plum: '#673158',
}
...
export default function AanchaliDesignDetail({ collectionSlug, designSlug }) {
  const collection = getAanchaliPattuCollection(collectionSlug)
  const design = getAanchaliPattuDesign(collectionSlug, designSlug)

  const galleryImages = useMemo(() => {
    if (!collection || !design) return []
    return [...new Set([design.image, ...(design.images || [])])].filter(Boolean).slice(0, 4)
  }, [collection, design])
    ...
    return (
      <section className="bg-[#f6efe4] px-5 py-32 text-[#1a1420] sm:px-8 lg:px-16">
        <div className="mx-auto max-w-5xl text-center">
          ...
          <a href="/Aanchali-pattu" className="mt-8 inline-flex items-center gap-3 rounded bg-[#b57922] px-5 py-3 font-sans text-[11px] font-bold uppercase tracking-[1.8px] text-white">
            Back to Aanchali Pattu<i className="fas fa-arrow-right" />
          </a>
        </div>
      </section>
    )
    ...
    <section className="bg-[#080711] text-white">
      <div className="relative isolate overflow-hidden px-5 pb-12 pt-10 sm:px-8 lg:px-16">
        ...
        <div className="mx-auto max-w-7xl">
          <a href={`/Aanchali-pattu/${collection.slug}`} className="inline-flex border-b border-[#c9933a] pb-1 font-sans text-[10px] font-bold uppercase tracking-[2.5px] text-[#c9933a] transition-colors hover:text-[#f0c36e]">
          ...
          <div className="relative mx-auto mt-14 max-w-7xl">
            ...
            <div className="mt-8 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {relatedDesigns.slice(0, 3).map((item) => (
                <a key={item.slug} href={`/Aanchali-pattu/${collection.slug}/${item.slug}`} className="group overflow-hidden saree-info-card rounded-lg border border-[#d9b77d] bg-[#fffaf2]/92 shadow-[0_20px_50px_rgba(116,73,28,0.12)] transition-transform hover:-translate-y-1">
