...
export default function SezhinoolDesignDetail({ collectionSlug, designSlug }) {
  const collection = getSezhinoolPattuCollection(collectionSlug)
  const design = getSezhinoolPattuDesign(collectionSlug, designSlug)
    ...
    return (
      <section className="bg-[#f6efe4] px-5 py-32 text-[#1a1420] sm:px-8 lg:px-16">
        <div className="mx-auto max-w-5xl text-center">
          ...
          <a href="/sezhinool-pattu" className="mt-8 inline-flex items-center gap-3 rounded bg-[#b57922] px-5 py-3 font-sans text-[11px] font-bold uppercase tracking-[1.8px] text-white">
            Back to Sezhinool Pattu <i className="fas fa-arrow-right" />
          </a>
        </div>
      </section>
    )
    ...
    <section className="bg-[#080711] text-white">
      <div className="relative isolate overflow-hidden px-5 pb-12 pt-32 sm:px-8 lg:px-16">
        ...
        <div className="mx-auto max-w-7xl">
          <a href={`/sezhinool-pattu/${collection.slug}`} className="inline-flex border-b border-[#c9933a] pb-1 font-sans text-[10px] font-bold uppercase tracking-[2.5px] text-[#c9933a] transition-colors hover:text-[#f0c36e]">
          ...
          <div className="relative mx-auto mt-14 max-w-7xl">
            ...
            <div className="mt-8 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {relatedDesigns.slice(0, 3).map((item) => (
                <a key={item.slug} href={`/sezhinool-pattu/${collection.slug}/${item.slug}`} className="group overflow-hidden saree-info-card rounded-lg border border-[#d9b77d] bg-[#fffaf2]/92 shadow-[0_20px_50px_rgba(116,73,28,0.12)] transition-transform hover:-translate-y-1">
