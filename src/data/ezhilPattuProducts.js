const ezhilProductImages = {
  amberThangaThorana: {
    card: '/ezhil-pattu/EZ-TT-0101-3.png',
    hero: '/ezhil-pattu/EZ-TT-0101-3.png',
    gallery: [
      '/ezhil-pattu/EZ-TT-0101-3.png',
      '/ezhil-pattu/EZ-TT-0101-4.png',
      '/ezhil-pattu/EZ-TT-0101-1.png',
      '/ezhil-pattu/EZ-TT-0101-2.png',
    ],
  },
  rubyThangaThorana: {
    card: '/ezhil-pattu/thangathoranai-4/EZ-TT-0102-3.png',
    hero: '/ezhil-pattu/thangathoranai-4/EZ-TT-0102-3.png',
    gallery: [
      '/ezhil-pattu/thangathoranai-4/EZ-TT-0102-3.png',
      '/ezhil-pattu/thangathoranai-4/EZ-TT-0102-4.png',
      '/ezhil-pattu/thangathoranai-4/EZ-TT-0102-1.png',
      '/ezhil-pattu/thangathoranai-4/EZ-TT-0102-2.png',
    ],
  },
  tealThangaThorana: {
    card: '/ezhil-pattu/thangathorana-3/EZ-TT-0103-3.png',
    hero: '/ezhil-pattu/thangathorana-3/EZ-TT-0103-3.png',
    gallery: [
      '/ezhil-pattu/thangathorana-3/EZ-TT-0103-3.png',
      '/ezhil-pattu/thangathorana-3/EZ-TT-0103-4.png',
      '/ezhil-pattu/thangathorana-3/EZ-TT-0103-1.png',
      '/ezhil-pattu/thangathorana-3/EZ-TT-0103-2.png',
    ],
  },
  violetThangaThorana: {
    card: '/ezhil-pattu/thangathoranai-2/EZ-TT-0104-3.png',
    hero: '/ezhil-pattu/thangathoranai-2/EZ-TT-0104-3.png',
    gallery: [
      '/ezhil-pattu/thangathoranai-2/EZ-TT-0104-3.png',
      '/ezhil-pattu/thangathoranai-2/EZ-TT-0104-4.png',
      '/ezhil-pattu/thangathoranai-2/EZ-TT-0104-1.png',
      '/ezhil-pattu/thangathoranai-2/EZ-TT-0104-2.png',
    ],
  },
}

export const ezhilPattuCollections = [
  {
    slug: 'azhagu-pattu',
    altSlugs: ['thanga-thorana-pattu', 'thanga-thoranai-pattu'],
    name: 'தங்க தோரண பட்டு',
    eyebrow: 'Golden Festive Borders - Ezhil Pattu',
    description: 'Thanga Thorana Pattu features warm festive silk shades, paisley-inspired woven motifs, and gold zari borders made for celebrations, gifting, and graceful traditional draping.',
    cardImage: ezhilProductImages.amberThangaThorana.card,
    heroImage: '/ezhil-pattu/thangathoranai-4/EZ-TT-0102-4.png',
    heroImages: [
      '/ezhil-pattu/thangathoranai-4/EZ-TT-0102-4.png',
      '/ezhil-pattu/thangathorana-3/EZ-TT-0103-4.png',
      '/ezhil-pattu/thangathoranai-2/EZ-TT-0104-3.png',
    ],
    designs: [
      {
        slug: 'amber-thanga-thorana-silk',
        altSlugs: ['teal-azhagu-silk'],
        name: 'Amber Thanga Thorana Silk',
        code: 'EZ-TT-0101',
        cardImage: ezhilProductImages.amberThangaThorana.card,
        image: ezhilProductImages.amberThangaThorana.gallery[0],
        images: ezhilProductImages.amberThangaThorana.gallery,
        price: 'Rs. 17,200',
        oldPrice: 'Rs. 23,900',
        discount: '28% off',
        stock: 'In stock',
        color: 'Amber Gold',
        colors: ['Amber Gold', 'Magenta Pink', 'Gold Zari'],
        fabric: 'Pure silk saree in vibrant amber gold with magenta pink contrast border',
        zari: 'Gold zari paisley motifs with rich magenta pink border and pallu',
        occasion: 'Festivals, pooja functions, engagement gifting, and family celebrations',
        blouse: 'Matching magenta pink silk blouse piece with gold zari border accent',
        details: 'Amber Thanga Thorana Silk from Ezhil Pattu brings a bright amber-gold body, woven paisley buttas, antique gold zari, and a rich magenta-pink border accent for a polished festive drape.',
      },
      {
        slug: 'ruby-thanga-thorana-silk',
        altSlugs: ['emerald-thanga-thorana-silk'],
        name: 'Emerald Thanga Thorana Silk',
        code: 'EZ-TT-0102',
        cardImage: ezhilProductImages.rubyThangaThorana.card,
        image: ezhilProductImages.rubyThangaThorana.gallery[0],
        images: ezhilProductImages.rubyThangaThorana.gallery,
        price: 'Rs. 18,400',
        oldPrice: 'Rs. 25,600',
        discount: '28% off',
        stock: 'In stock',
        color: 'Emerald Green',
        colors: ['Emerald Green', 'Coral Red', 'Gold Zari'],
        fabric: 'Pure silk saree in rich emerald green with coral red contrast border',
        zari: 'Antique gold thoranam-inspired zari border and rich red woven pallu',
        occasion: 'Wedding, muhurtham, reception, and festive celebrations',
        blouse: 'Coral red silk blouse piece with antique gold zari edging',
        details: 'Emerald Thanga Thorana Silk combines a rich emerald-green body with traditional paisley buttas and an antique-gold zari red contrast border.',
      },
      {
        slug: 'teal-thanga-thorana-silk',
        altSlugs: ['royal-blue-thanga-thorana-silk'],
        name: 'Royal Blue Thanga Thorana Silk',
        code: 'EZ-TT-0103',
        cardImage: ezhilProductImages.tealThangaThorana.card,
        image: ezhilProductImages.tealThangaThorana.gallery[0],
        images: ezhilProductImages.tealThangaThorana.gallery,
        price: 'Rs. 17,900',
        oldPrice: 'Rs. 24,900',
        discount: '28% off',
        stock: 'In stock',
        color: 'Royal Blue',
        colors: ['Royal Blue', 'Sapphire Blue', 'Gold Zari'],
        fabric: 'Lustrous pure silk saree in deep royal blue with finely woven festive motifs',
        zari: 'Gold zari border with floral thoranam detailing',
        occasion: 'Engagement, reception, pooja, and family functions',
        blouse: 'Royal-blue silk blouse piece with coordinated gold border',
        details: 'Royal Blue Thanga Thorana Silk offers a luminous sapphire-blue shade framed by graceful gold zari for refined celebrations.',
      },
      {
        slug: 'violet-thanga-thorana-silk',
        altSlugs: ['golden-olive-thanga-thorana-silk'],
        name: 'Golden Olive Thanga Thorana Silk',
        code: 'EZ-TT-0104',
        cardImage: ezhilProductImages.violetThangaThorana.card,
        image: ezhilProductImages.violetThangaThorana.gallery[0],
        images: ezhilProductImages.violetThangaThorana.gallery,
        price: 'Rs. 18,200',
        oldPrice: 'Rs. 25,300',
        discount: '28% off',
        stock: 'In stock',
        color: 'Golden Olive',
        colors: ['Golden Olive', 'Maroon Wine', 'Antique Gold'],
        fabric: 'Pure silk in golden olive tone with woven paisley motifs and a smooth festive drape',
        zari: 'Antique gold zari with a rich maroon contrast border and pallu',
        occasion: 'Reception, engagement, festive gifting, and celebrations',
        blouse: 'Maroon silk blouse piece with contrast zari edging',
        details: 'Golden Olive Thanga Thorana Silk pairs a warm golden-olive body with antique-gold weaving and a rich maroon contrast finish.',
      },
    ],
  },
]

export const getEzhilPattuCollection = (slug) =>
  ezhilPattuCollections.find((c) => c.slug === slug || c.altSlugs?.includes(slug))

export const getEzhilPattuDesign = (collectionSlug, designSlug) => {
  const collection = getEzhilPattuCollection(collectionSlug)
  return collection?.designs.find((d) => d.slug === designSlug || d.altSlugs?.includes(designSlug))
}
